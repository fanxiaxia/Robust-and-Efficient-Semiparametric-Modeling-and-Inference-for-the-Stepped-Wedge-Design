...
if("lmm_simple" %in% est)
{
  ...
  lmmfit.c <- glmmTMB(y ~  x + factor(time) + (1|cluster) , data = dataset)
  lmmfit.f <- glmmTMB(y ~  x + factor(time) +  (1 + factor(time)|cluster), data = dataset)
  # Warning messages:
...
write.csv(res,"res_simple.csv")
write.csv(res.lmm,"res_lmm_simple.csv")
